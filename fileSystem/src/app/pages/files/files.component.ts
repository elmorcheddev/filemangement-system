...
addAcces(id: number) {
  ...
  addNewDossier(form: NgForm) {
    this.catService.saveNewCat(this.cat).subscribe((data: Cat) => {
      console.log(data)
      if (data !== null) {
        window.alert("dossier ajouter avec success")
        this.active = false
              this.getAllCat()
        const id = data.id
  ...
  uploadFile(form: NgForm) {
    if (form.valid && this.selectedFile) {
      ...
      formData.append('fichier', new Blob([JSON.stringify(this.fichier)], { type: 'application/json' }));

      formData.append('catId', this.cat.id.toString());
      formData.append('userId', this.utilisateur.id.toString());

      this.fichierService.uploadFile(formData).subscribe((data: Fichier) => {
        if (data !== null) {
         
           console.log(data)
          window.alert ("Files added with success ") 
            location.reload()
